...
- **41 data-literacy gotchas.** Scales are mixed within a single row, `position` means rank-among-tracked-brands not overall, `list_models` shows 19 engines but only a handful are active, the sentiment formula isn't what the docs imply, `get_actions` is currently broken (with a worked-out local workaround), `retrieval_rate` and `citation_rate` can exceed 1.0, domain and URL reports return different types for the same-named columns, engine-returned empty response bodies silently inflate non-mention counts, `get_brand_report` dimension labels can return null on queued projects, `list_search_queries` (fanout) returns zero rows for AI Overview, AI Mode, and Copilot, and is confirmed to return data for ChatGPT and Grok (other engines untested), and more.
